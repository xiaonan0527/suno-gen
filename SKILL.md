...
脚本会查询该任务状态，如果已完成则直接下载，未完成则继续轮询。

> 脚本自动完成：提交/查询任务 → 轮询状态 → 下载所有曲目的音频/封面/歌词/视频到本地 → stdout 输出各音频文件路径（每行一个）。
>
> Suno 每次生成 **2 首曲目**，脚本会为每首曲目分别下载，文件名自动加后缀 `_1`、`_2`。

### 判断使用哪种模式

- 用户说"重新下载"/"下载之前的"/"用这个 task_id" → **模式 B**，用 `--task-id`
- 用户只给了风格/主题描述（如"写一首关于春天的歌"）→ **模式 A 灵感模式**，不加 `--custom`
- 用户提供了具体歌词 → **模式 A 自定义模式**，加 `--custom`，歌词作为 `--prompt`
